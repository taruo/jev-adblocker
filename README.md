...
API キーはこの Chrome プロファイルの拡張ストレージに保存され、ページ全体は送信されません。候補要素の短い本文と属性（タグ、クラス、ラベル、短いテキスト、クエリ文字列を除いた URL）が拡張からTypeSafeへ直接送信されます。

## 判定の流れ

候補を最大20件ずつ一つの System One リクエストへまとめ、候補ごとに独立した Noul 質問を作ります。返った `noul`（広告である確率）が初期しきい値 0.78 以上なら要素を非表示にします。しきい値はポップアップから 50〜99% に変更できます。401・キー未設定では自動再試行を停止し、通信失敗・429・529は回数制限付きバックオフを使います。API が失敗したときは候補を隠さず、固定ドメインルールだけを使います。
